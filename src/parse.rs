...
impl AstNode {
  ...
  fn children(&self) -> Option<(&Box<AstNode>, &Box<AstNode>)> {
    match self {
      Self::Leaf { .. } | Self::Empty => None,
      Self::BinOp { left, right, .. } => Some((&left, &right)),
    }
  }

  fn children_mut(&mut self) -> Option<(&mut Box<AstNode>, &mut Box<AstNode>)> {
    match self {
      Self::Leaf { .. } | Self::Empty => None,
      Self::BinOp {
        ref mut left,
...
impl PartialOrd for AstNode {
  fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
    let paren_nesting1 = self.paren_level();
    let paren_nesting2 = other.paren_level();
    if paren_nesting1 != paren_nesting2 {
      return paren_nesting1.partial_cmp(&paren_nesting2);
    }
    ...
    loop {
      if ptr.is_empty() {
        ...
        *ptr = node;
        ...
        // Node has lower precedence, node becomes parent of ptr
        let (left, right) = node.children_mut().unwrap();
        *right = Box::new(AstNode::Empty);
        *left = ptr.clone();
        *ptr = node;
        break;
